# Put here so it's on github, but only ever run from ~/DNABERT_2/finetune due to dependencies
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, 
                 ...
                 kmer: int = -1, task: str = "classification"): # NEW: ADD TASK
...
def preprocess_logits_for_metrics(logits:Union[torch.Tensor, Tuple[torch.Tensor, Any]], labels):
    if isinstance(logits, tuple): # Unpack logits if it's a tuple
        logits = logits[0]

    ########## NEW: SKIP ARGMAX IF TASK IS REGRESSION ############
    if labels.dtype in [torch.float, torch.float32, torch.float64]:
        return logits.squeeze()

    # Otherwise, classification: use argmax
    if logits.ndim == 3:
...
def compute_metrics(eval_pred):
    ...
    return calculate_metric_with_sklearn(predictions, labels) # Otherwise: assume classification
...
def train():
    ...
    test_dataset = SupervisedDataset(tokenizer=tokenizer, 
                                     ...
                                     kmer=data_args.kmer, task=training_args.task)
    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer, regression=(training_args.task == "regression"))
